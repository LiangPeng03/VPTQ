...
K-means算法的核心思想是通过迭代过程最小化簇内数据点到其质心的距离平方和。算法包含以下步骤：

1. **初始化**：随机选择K个质心
2. **分配**：将每个数据点分配给最近的质心
3. **更新**：重新计算每个簇的质心（即该簇中所有点的平均值）
4. **重复**：重复步骤2和3直到收敛


### 3.2. 距离计算方法

在VPTQ的实现中，使用了欧几里得距离来计算向量之间的距离：

`dist = torch.cdist(data.float(), self.centroids[cidx].float())`

#### 3.2.1.  欧几里得距离公式

对于两个向量 x = (x₁, x₂, ..., xₙ) 和 y = (y₁, y₂, ..., yₙ)，欧几里得距离计算公式为：

$d(x, y) = √[(x₁-y₁)² + (x₂-y₂)² + ... + (xₙ-yₙ)²]$

在VPTQ中，使用PyTorch的`torch.cdist`函数计算距离矩阵，该函数可以高效地计算批量数据点到所有质心的距离。

==其中欧几里得距离涉及了开根，K-Means涉及平方，理论上二者可以抵消，但他们都是完整封装且经过底层优化的，不知道能否在这一点上进行优化，需要查看二者的底层代码，是否支持拆解并去除冗余==

## 4. hessian

- **VPTQ**: 计算协方差矩阵（均值已中心化），公式为 `H = E[xx^T] - E[x]E[x]^T`
- **GPTQ**: 计算未中心化的二阶矩矩阵，公式为 `H = E[xx^T]`
...
逆矩阵
	...
	将量化误差通过逆Hessian矩阵变换，得到在考虑了权重间相关性后的校正项。
