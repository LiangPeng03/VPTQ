...
# 创建输出目录（基于当前时间）
OUTPUT_DIR="/home/pengliang/Desktop/VPTQ/outputs/opt_125M/$(date +%Y-%m-%d-%H-%M-%S)"
mkdir -p "$OUTPUT_DIR"
...
CUDA_VISIBLE_DEVICES=0,1 /home/pengliang/glibc/glibc-2.32-install/lib/ld-linux-x86-64.so.2 \
  ...
  $(which python) run_vptq.py \
    ...
    --output_dir /home/pengliang/Desktop/VPTQ/outputs/facebook--opt-125m/ \
    --vector_lens -1 8 \
    --group_num 1 \
    ...
    --save_model \
    --save_packed_model \
    --hessian_path /home/pengliang/Desktop/local_models/Hessians-OPT-125M-6144-8k/hessian \
    --inv_hessian_path /home/pengliang/Desktop/local_models/Hessians-OPT-125M-6144-8k/invhessian \
    --ktol 1e-5 --kiter 100 
